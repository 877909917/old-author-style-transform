package r1a;
 ...
 public class B {
 	...
 	class Config {
 		...
 		public static final String IN_FILE = "B-small-attempt2.in";
 	...
 	class ParallelSolver implements Runnable {
 		...
 		long floor(long a, long b) { return a / b; }
 		...
 		public void run() {
 			...
 			LOOP: for (int i = 0; i < p; i++) {
 				...
 				for (int j = 0; j < n; j++) {
 					while (cur[j] < p && q[j][cur[j]] * 10 < r[j] * low * 9) cur[j]++;
 					if (cur[j] >= p) break LOOP;
 					low = Math.max(low, ceil(q[j][cur[j]] * 10, r[j] * 11));
 					high = Math.min(high, floor(q[j][cur[j]] * 10, r[j] * 9));
 				}
 
 				cur[0]++;
 				if (low <= high) {
 					ans++;
 					for (int j = 1; j < n; j++) cur[j]++;
 				}
 			}
 			out.println(ans);
 		}
 		
 		int func1(long q, long r) {
 			int v1 = 0;
 			for (int k = 30; k >= 0; k--) {
 				final long v = 1<<k;
 				if (q*10 > r*(v+v1)*11) v1 += v;
 			}
 			return v1 + 1;
 		}
 		
 		int func2(long q, long r) {
 			int v1 = 0;
 			for (int k = 30; k >= 0; k--) {
 				final long v = 1<<k;
 				if (q*10 >= r*(v+v1)*9) v1 += v;
 			}
 			return v1;
 		}
