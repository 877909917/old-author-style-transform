 ...
 public class B {
 	...
 	String calc(int n1, int n2, int n3, char c1, char c2, char c3) {
 		if (n1 > n2 + n3) return "";
 	...
 	public void run() {
 		...
 		for (int caseN = 1; caseN <= T; caseN++) {
 			...
 			String RYB = calc(R, Y, B, 'R', 'Y', 'B');
 			String YRB = calc(Y, R, B, 'Y', 'R', 'B');
 			String BRY = calc(B, R, Y, 'B', 'R', 'Y');
 			
 			StringBuilder res = new StringBuilder();
 			if (RYB.length() != 0) res.append(RYB);
 			else if (YRB.length() != 0) res.append(YRB);
 			else if (BRY.length() != 0) res.append(BRY);
 			
 			out.println("Case #" + caseN + ": " + (res.length() == 0 ? "IMPOSSIBLE" : res));
 		}
 		out.close();
 	}
