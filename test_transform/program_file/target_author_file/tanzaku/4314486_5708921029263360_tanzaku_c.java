package R1C;
 ...
 public class C {
 	...
 	class Config {
 		...
 		public static final String BASE_DIR = "data\\R1C" ;
 	...
 	class ParallelSolver implements Runnable {
 				...
 				if(cur > ans) {
 					...
 					for(int i = 0; i < J; i++) {
 						for(int j = 0; j < S; j++) {
 							if((bs[i] >>> j & 1) == 1) {
 								for(int[] ls : mp.get(j)) {
 									res.add(new int[]{i+1,ls[0]+1,ls[1]+1});
 								}
 							}
 						}
 					}
 		...
 		void naive() {
 			int[] ps = new int[J*P*S];
 			for(int i = 0, ix = 0; i < J; i++) {
 				for(int j = 0; j < P; j++) {
 					for(int k = 0; k < S; k++) {
 						ps[ix++] = (i * 10 + j) * 10 + k;
 					}
 			...
 			int cnt = 0;
 			do {
 				int[][] pair1 = new int[J][P];
 				int[][] pair2 = new int[P][S];
 				int[][] pair3 = new int[J][S];
 				int c = 0;
 				for(int p : ps) {
 					int i = p / 100;
 					int j = p / 10 % 10;
 					int k = p % 10;
 					if(++pair1[i][j] > K || ++pair2[j][k] > K || ++pair3[i][k] > K) {
 						break;
 					}
 					c++;
 				}
